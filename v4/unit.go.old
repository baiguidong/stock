...
func merg_image(srcs []string, des string) {
	...
	lens = lens - emptynum
	ww := (width+2)*lens + a_head + 10*emptynum
	// if jb >= 30 {
	// 	ww = (width+2)*lens/2 + (g_ks+2)*lens/2 + a_head + 10*emptynum
	// }
	//ww := (width+2)*lens + a_head + 10*emptynum
	hh := high*3 + h_head + s_head*2
	...
	for n, src := range srcs {
		if src == "empty.png" {
			...
			if err == nil {
				...
				// } else {
				dst = imaging.Paste(dst, s1, image.Pt(a_head/2+(n-a)*(width+2)+a*10, 0))
				//}
			}
		}
	}
...
func merg_image_h(srcs []string, des string) {
	...
	for n, src := range srcs {
		...
		if err != nil {
			...
			if n == 0 {
				s1 = imaging.Resize(s1, width, h_head, imaging.Linear)
				dst = imaging.Paste(dst, s1, image.Pt(0, h_head))
			} else {
				s1 = imaging.Resize(s1, width, high, imaging.Linear)
				if n == 1 {
					//n*400-200+4*n
...
func merg_image_h_120(srcs []string, des string) {
	//lens := len(srcs)
	dst := imaging.New(g_ks, high*(3)+4*h_head, color.NRGBA{255, 255, 255, 255})
	for n, src := range srcs {
		s1, err := imaging.Open(src)
		if err != nil {
			fmt.Println(err.Error())
			...
			if n == 0 {
				...
				dst = imaging.Paste(dst, s1, image.Pt(0, 2*h_head+4+high/2))
			} else {
				s1 = imaging.Resize(s1, g_ks, g_gs, imaging.Linear)
				if n == 1 {
					//n*400-200+4*n
					dst = imaging.Paste(dst, s1, image.Pt(0, 2*h_head+8+g_gs+high/2))
				}
			}
		}
	}
...
func get_p_120(dt plotter.XYs, name, title string) {
	...
	// if jb <= 60 {
	// 	p.Add(
	// 		get_line_new(0, mi, mx),
	// 		get_line_new(30, mi, mx), l)
	// } else if jb <= 90 {
	// 	p.Add(
	// 		get_line_new(0, mi, mx),
	// 		get_line_new(30, mi, mx),
	// 		get_line_new(60, mi, mx), l)
	// } else if jb <= 120 {
	// 	p.Add(
	// 		get_line_new(0, mi, mx),
	// 		get_line_new(30, mi, mx),
	// 		get_line_new(60, mi, mx),
	// 		get_line_new(90, mi, mx), l)
	// } else if jb <= 150 {
	// 	p.Add(
	// 		get_line_new(0, mi, mx),
	// 		get_line_new(30, mi, mx),
	// 		get_line_new(60, mi, mx),
	// 		get_line_new(90, mi, mx),
	// 		get_line_new(120, mi, mx), l)
	// } else if jb <= 180 {
	// 	p.Add(
	// 		get_line_new(0, mi, mx),
	// 		get_line_new(30, mi, mx),
	// 		get_line_new(60, mi, mx),
	// 		get_line_new(90, mi, mx),
	// 		get_line_new(120, mi, mx),
	// 		get_line_new(150, mi, mx), l)
	// } else if jb <= 210 {
	// 	p.Add(
	// 		get_line_new(0, mi, mx),
	// 		get_line_new(30, mi, mx),
	...
	// 		get_line_new(90, mi, mx),
	// 		get_line_new(120, mi, mx),
	// 		get_line_new(150, mi, mx),
	// 		get_line_new(180, mi, mx), l)
	// } else if jb <= 240 {
	// 	p.Add(
	// 		get_line(0, mi, mx),
	// 		get_line(30, mi, mx),
