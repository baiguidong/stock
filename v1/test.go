...
var g_day *string = flag.String("day", "2019-04-03", "Use -day <2018-09-18>")
var g_py *int = flag.Int("py", 71, "Use -py 0")
var g_jb *int = flag.Int("jb", 60, "Use -jb 240")
var g_ty *int = flag.Int("ty", 10, "Use -ty 1")
...
// 输出原图 [C:\soft\杨海\客户端\svr_1.exe -day=2020-07-14 -jb=240 -py=10 -gd=200 -kd=600 -xt=3 -gz=350 -gm=60 -ty=1]
// 输出全图 [C:\soft\杨海\客户端\svr_1.exe -day=2020-07-14 -jb=240 -py=0 -gd=200 -kd=600 -xt=3 -gz=350 -gm=60 -ty=5]
...
func f5(i chan int) {
	...
	if len(res_vec2) > 0 {
		...
		for _, s := range res_vec2 {
			tmp = append(tmp, s)
			if len(tmp) >= g_num {
				name := fmt.Sprintf(get_home_dir()+"/比对结果_%d_a_%d.png", g_ty_i, num)
				os.Remove(name)
		...
		if len(tmp) > 0 {
			name := fmt.Sprintf(get_home_dir()+"/比对结果_%d_a_%d.png", g_ty_i, num)
			os.Remove(name)
			if (g_ty_i == 6 || g_ty_i == 4 || g_ty_i == 5 || g_ty_i == 10) && *g_py > 0 {
				merg_image_4(tmp, name)
			} else {
				merg_image(tmp, name)
			}
...
func f2(i chan int) {
	for n, rs := range ads_all.data {
		...
		fname := fmt.Sprintf("tmp/out_h_%d.png", n)
		if (g_ty_i == 6 || g_ty_i == 5 || g_ty_i == 10) && *g_py > 0 {
			aaa = append(aaa, name_p)
		}
		merg_image_h(aaa, fname)
		a1 = append(a1, fname)
